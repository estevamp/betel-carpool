...
export function TourSpotlight({
  ...
  // Whether the card has been positioned at least once (skip transition on first placement)
  ...
  // Reset positioned flag when tour opens so the first position snaps without transition
  useEffect(() => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* ── SVG overlay — path morphs to new spotlight position ── */}
          <motion.svg
            key="tour-svg"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          {/* ── Tooltip card ──
              Position is applied via inline style + CSS transition.
              Framer Motion only handles the initial scale/fade on mount/unmount.
              This avoids the "slides from 0,0" bug that happens when Framer
              animates top/left starting from undefined.
          ── */}
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                style={{
                  width: cardWidth,
                  // CSS transition for smooth glide between positions (not Framer)
                  transition: hasPositioned
                    ? "top 0.35s cubic-bezier(0.34,1.2,0.64,1), left 0.35s cubic-bezier(0.34,1.2,0.64,1)"
                    : "none",
                  ...(tooltipPos
