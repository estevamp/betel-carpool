...
import { Settings, Wallet, Bell, Shield, Calendar, Clock, MessageSquare, Lock } from "lucide-react";
...
export default function ConfiguracoesPage() {
  ...
  // Pre-trip notification settings
  const [preTripNotifEnabled, setPreTripNotifEnabled] = useState(false);
  const [preTripNotifMinutes, setPreTripNotifMinutes] = useState("60");
  ...
  useEffect(() => {
    if (settings) {
      ...
      const preTripEnabled = settings.find((s) => s.key === "pre_trip_notification_enabled");
      if (preTripEnabled) setPreTripNotifEnabled(preTripEnabled.value === "true");

      const preTripMins = settings.find((s) => s.key === "pre_trip_notification_minutes");
      if (preTripMins) setPreTripNotifMinutes(preTripMins.value);
      ...
      setTripLockHours("2");
      setPreTripNotifEnabled(false);
      setPreTripNotifMinutes("60");
    }
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (!effectiveCongregationId) throw new Error("Nenhuma congregação selecionada");
      ...
      const upsertSetting = async (key: string, value: string, type: string) => {
        ...
        if (existing) {
          ...
          const { error } = await supabase
            .from("settings")
            .insert({ key, value, type, congregation_id: effectiveCongregationId });
          if (error) throw error;
        }
      };
      ...
      await upsertSetting("pre_trip_notification_enabled", preTripNotifEnabled.toString(), "boolean");
      await upsertSetting("pre_trip_notification_minutes", preTripNotifMinutes, "integer");

      if (isSuperAdmin && defaultCongregationId) {
        await supabase.from("settings").delete().eq("key", "default_congregation_id");
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["settings", effectiveCongregationId] });
  ...
  const saveNotificationMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["notification-settings", effectiveCongregationId] });
          ...
          <div className="p-5 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <ToggleGroup
                ...
                disabled={!effectiveCongregationId}
              >
                {[["0","D"],["1","S"],["2","T"],["3","Q"],["4","Q"],["5","S"],["6","S"]].map(([val, label]) => (
                  <ToggleGroupItem
                    key={val}
                    value={val}
                    className="w-10 h-10"
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-border space-y-4">
                ...
                {tripLockEnabled && (
                  <div className="grid gap-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Motoristas não poderão editar a viagem dentro desse intervalo. Apenas o admin poderá
                      fazer alterações.
                    </p>
                  </div>
                )}
              ...
              <div className="pt-4 border-t border-border space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-foreground">Notificação Pré-Viagem</h3>
                </div>

                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label htmlFor="preTripEnabled">Habilitar notificação pré-viagem</Label>
                    <p className="text-sm text-muted-foreground">
                      Envia lembrete automático para motoristas e passageiros antes de cada viagem.
                    </p>
                  </div>
                  <Switch
                    id="preTripEnabled"
                    checked={preTripNotifEnabled}
                    onCheckedChange={setPreTripNotifEnabled}
                    disabled={!effectiveCongregationId}
                  />
                </div>
                ...
                {preTripNotifEnabled && (
                  <div className="grid gap-2">
                    <Label htmlFor="preTripMinutes">
                      Quantos minutos antes da viagem enviar o lembrete?
                    </Label>
                    <div className="flex items-center gap-3">
                      <Input
                        id="preTripMinutes"
                        ...
                        max="1440"
                        step="5"
                        value={preTripNotifMinutes}
                        onChange={(e) => setPreTripNotifMinutes(e.target.value)}
                    ...
                    <p className="text-xs text-muted-foreground">
                      Ex.: 60 = 1 hora antes. O motorista recebe a quantidade de passageiros;
                      cada passageiro recebe o nome do motorista e o horário da partida.
                    </p>
                  </div>
                )}
              </div>
