...
export default function Dashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data
      } = await supabase.from("settings").select("value").eq("key", "congregation_name").maybeSingle();
  ...
  const {
    ...
    queryFn: async () => {
      const {
        ...
        `).eq("is_active", true).order("departure_at", {
        ascending: true
      });
  ...
  const subtitle = congregationName ? `Sistema de Transporte de Betelitas - ${congregationName}` : "Bem-vindo ao sistema de transporte de Betelitas";
  return <motion.div className="space-y-8 max-w-4xl mx-auto" variants={containerVariants} initial="hidden" animate="visible">
      {/* Header */}
      ...
      <motion.div variants={itemVariants}>
        <div className="bg-card rounded-xl border border-border shadow-card overflow-hidden">
          ...
          <div className="divide-y divide-border">
            ...
            return <div key={trip.id} className="flex items-center gap-4 px-5 py-4 hover:bg-muted/50 transition-colors">
                    ...
                    <span className={cn("px-2.5 py-1 rounded-full text-xs font-medium", availableSeats > 0 ? "bg-success/10 text-success" : "bg-muted text-muted-foreground")}>
      ...
      <motion.div variants={itemVariants}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {secondaryItems.map(item => <Link key={item.path} to={item.path} className="flex items-center gap-3 p-4 bg-card rounded-xl border border-border shadow-card hover:bg-muted/50 transition-colors">
